...
if(tg){tg.ready();tg.expand();tg.setHeaderColor("#09051d");tg.setBackgroundColor("#070314");tg.enableClosingConfirmation?.();}
...
function loadHigh(){const local=Number(localStorage.getItem("cosmic-high")||0);ui.highScore.textContent=local;tg?.CloudStorage?.getItem("cosmic-high",(_,v)=>{if(v)ui.highScore.textContent=Math.max(local,Number(v))})}
